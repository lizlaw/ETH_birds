...
library(nimbleEcology)
 library(nimbleDistance) #  if using halfnorm function
library(tidyverse)
...
trialName <- "HN"  # hyperpriors priors inits library params manually altered. 
```
...
occupancy_code <- nimbleCode({
  ...
  # Hyperpriors for the model of occupancy (psi)
    ...
    # prior for elevation polynomial
     ...
     #  mu.betalpsi1Rate ~ dunif(0, 6) # the rate parameter gamma- uniform prior
     # lognorm version 
     #  mu.betalpsi[1] ~ dunif(-1,1) # the mu parameter lognorm- uniform prior
     # mu.betalpsi1sd ~ dunif(0, 1) # the sd parameter lognorm- uniform prior
     # halfnorm version  
       mu.betalpsi[1] ~ dunif(1, 12) # the sigma parameter halfnorm- uniform prior
  ...
  for(k in 1:M){   
    ...
    for (lpsii in 2:npsi){
      ...
      #betalpsi[k, 1] ~ dgamma(shape=mu.betalpsi[1], 
      #                        rate=mu.betalpsi1Rate) 
      # lognorm distribution for the polynomial
      #betalpsi[k, 1] ~ dlnorm(meanlog=mu.betalpsi[1], 
      #                        sdlog=mu.betalpsi1sd) 
      # halfnorm distribution for the polynomial
      betalpsi[k, 1] ~ dHN_V(sigma=mu.betalpsi[1]) 
...
# betalpsi[1:M, 1] <- rlnorm(n = M, meanlog=0, sdlog=1) # lognorm version
 betalpsi[1:M, 1] <- rHN_V(n = M, sigma = 1) # halfnorm version
lp <- rnorm(n = M)
...
params1 <- c("lpsi", "betalpsi", "lp", "betalp", "psi", "p",
            # "mu.betalpsi", "mu.betalpsi1Rate")  # gamma
            #  "mu.betalpsi", "mu.betalpsi1sd") # lnorm
               "mu.betalpsi")  # halfnorm
```
...
```{r MCMCvis}
library(MCMCvis)
pg1 <- c("mu.betalpsi", "mu.betalpsi1sd", 
