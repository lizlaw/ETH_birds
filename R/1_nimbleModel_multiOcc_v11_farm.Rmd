...
trialName <- "NoPolyFVars"  # hyperpriors priors inits library params manually altered. 
```
...
occupancy_code <- nimbleCode({
  ...
  # prior for elevation polynomial
  # gamma version
  #  mu.betalpsi[1] ~ dunif(0, 6) # the shape parameter gamma- uniform prior
  #  sd.betalpsi1 ~ dunif(0, 6) # the rate parameter gamma- uniform prior
  # lognorm version 
  #mu.betalpsi[1] ~ dunif(-5, 5) # the mu parameter lognorm- uniform prior
  #sd.betalpsi1 ~ dunif(0, 6) # the sd parameter lognorm- uniform prior
  # trunc norm version  
  #  mu.betalpsi[1] ~ dunif(0, 5) # the halfnorm mean -  uniform prior (use same sd as others)
  #  sd.betalpsi1  ~ dunif(0, 6) 
  # no quadratic version
    mu.betalpsi[1] <- 0 # the mu parameter 
    sd.betalpsi1 <- 0 # the sd parameter 
  ...
  for(k in 1:M){   
    ...
    # gamma distribution for the polynomial
    #betalpsi[k, 1] ~ dgamma(shape=mu.betalpsi[1], rate=sd.betalpsi1) 
    # lognorm distribution for the polynomial - 
    # betalpsi[k, 1] ~ dlnorm(meanlog=mu.betalpsi[1], sdlog=sd.betalpsi1)
    # truncnorm distribution for the polynomial
    #betalpsi[k, 1] ~ T(dnorm(mean = mu.betalpsi[1], sd = sd.betalpsi1), 0, Inf)
    # no quadratic version
    betalpsi[k, 1] <- 0
  ...
  for(k in 1:M){
    for (i in 1:nSite){
      ...
      logit(psi[i,k]) <- lpsi[k] + 
        #betalpsi[k,1] * Xoc[i,1] +  # elevation2
        betalpsi[k,2] * Xoc[i,2] +  # elevation1
      ...
      for(j in 1:nVisits){
        ...
        logit(p[i,j,k]) <- lp[k] + 
          betalp[k,1] * Xob[i,j,1] + # start2
          betalp[k,2] * Xob[i,j,2] + # date
          betalp[k,3] * Xob[i,j,3] + # start1
...
# 1"elevation2" 2"elevation1" 3"farm_85" 4"fl_dis" 5 "sidi1ha" 6"slope
Xoc <- OccVarsFarm %>% select(-point_id) %>% as.matrix() %>% array(dim = dim(.)) 
names(OccVarsFarm)
Xoc <- Xoc[,c(2,1,3:6)]  

# observation variables [site, visit, vars]
# 1"start2", 2"date",3"start1",4"visibility"
...
# betalpsi[1:M, 1] <- rgamma(n = M, shape = 1, rate = 1) # correct betalpsi to be rgamma
# betalpsi[1:M, 1] <- rlnorm(n = M, meanlog=0, sdlog=1) # lognorm version
# betalpsi[1:M, 1] <- abs(betalpsi[1:M, 1]) # truncnorm version
 betalpsi[1:M, 1] <- 0 # no quadratic version
...
params1 <- c("lpsi", "betalpsi", "lp", "betalp", "psi", "p")#,
             # "mu.betalpsi", "sd.betalpsi1")  # gamma
             #  "mu.betalpsi", "sd.betalpsi1") # lnorm
             #"mu.betalpsi", "sd.betalpsi1")  # truncnorm
```
...
```{r MCMCvis}
library(MCMCvis)
pg1 <- c(#"mu.betalpsi", "sd.betalpsi1", 
         "lpsi", "betalpsi", "lp", "betalp") 
