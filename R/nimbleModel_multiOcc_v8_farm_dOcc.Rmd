...
* implemented marginalized occupancy likelihood
...
occupancy_code <- nimbleCode({
  ...
  for (lpsii in 1:npsi){
    betalpsiRARE[lpsii] ~ dnorm(mu.betalpsi[lpsii], sd=sd.betalpsi)# [lpsii])
  }
  ...
  for(k in rareSpp:M){
    ...
    # detection - rare species specific
    lp[k] <- lpRARE
  }
  # detection, non-species specific
  for(lpi in 1:np){
...
post.dOcc <- readRDS(paste0(results_folder, version_folder, "BirdOccMod_dOcc_samples_farm_20220801.RDS"))
post.orig <- readRDS(paste0(results_folder, version_folder, "BirdOccMod_samples_farm_20220802.RDS"))

data.dOcc <- melt(do.call(rbind, post.dOcc))
data.dOcc$Model <- "dOcc"

data.orig <- melt(do.call(rbind, post.orig))
data.orig$Model <- "Original"

data.comb <- rbind(data.dOcc, data.orig)
colnames(data.comb) <- c("Sample", "Parameter", "Value", "Model")

plot.params <- c("mu.betalp", paste0("mu.betalpsi[", 1:3, "]"), 
                 "mu.lp", "mu.lpsi", "sd.betalpsi", "sd.lp", "sd.lpsi")

pdf(paste0(results_folder, version_folder, format(Sys.time(), "%Y%m%d"), "_ModelComparison_dOcc_orig.pdf"), width = 7, height = 5)
ggplot(subset(data.comb, Parameter %in% plot.params)) + 
