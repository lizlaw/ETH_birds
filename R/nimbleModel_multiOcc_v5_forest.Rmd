---
title: "bayesBirdModel_multiSPecies_v5 - forest"
author: "Liz Law"
date: "10 Jan 2022"
...
* Included only FOREST and select variables with transformations for this.
...
data_input_code <- "~/nimbleModel_multiOcc_v5_forestData.R"
...
occupancy_code <- nimbleCode({
  ...
  # Priors
    ...
    # Derived quantities
    for (i in 1:nSite){
      SR_site[i] <- sum(z[i,1:M])          # Number of occurring species at each site
    }
...
```{r save results}
saveRDS(samples, paste0(results_folder, version_folder,"BirdOccMod_samples_forest", format(Sys.time(), "%Y%m%d"), ".RDS"))
```
...
MCMCtrace(samples, 
          ...
          filename = paste0(results_folder, version_folder,"plots_traceDensity_pg1_forest", format(Sys.time(), "%Y%m%d"), ".pdf"))
MCMCtrace(samples, 
          params = pg2,
          ind = TRUE, 
          Rhat = TRUE, n.eff = TRUE,
          filename = paste0(results_folder, version_folder,"plots_traceDensity_pg2_forest", format(Sys.time(), "%Y%m%d"), ".pdf"))
