...
```{r import data}
source("/Users/elaw/Desktop/LeuphanaProject/BirdModelling/Leuphana_Bird_Modelling/nimbleMultiOcc/nimbleModel_multiOcc_v5_forestData.R")
```
...
```{r save results}
saveRDS(samples, paste0("BirdOccMod_samples_forest", format(Sys.time(), "%Y%m%d"), ".RDS"))
```
...
MCMCtrace(samples, 
          ...
          filename = paste0("plots_traceDensity_pg1_forest", format(Sys.time(), "%Y%m%d"), ".pdf"))
MCMCtrace(samples, 
          params = pg2,
          ind = TRUE, 
          Rhat = TRUE, n.eff = TRUE,
          filename = paste0("plots_traceDensity_pg2_forest", format(Sys.time(), "%Y%m%d"), ".pdf"))
