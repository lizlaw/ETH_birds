...
Xoc <- OccVarsFrst %>% select(-point_id) %>% as.matrix() %>% array(dim = dim(.))   # [site, vars]
...
# Set 1
params1 <- c("mu.lpsi", "sd.lpsi", "mu.betalpsi", "sd.betalpsi", "mu.lp", "sd.lp", "mu.betalp", "sd.betalp", "omega", "SR_site", "n0", "Ntotal")
