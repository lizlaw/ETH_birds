---
title: "bayesBirdModel_multiSPecies_v9 - forest"
author: "Liz Law"
date: "`r Sys.Date()`"
...
occupancy_code <- nimbleCode({
  ...
  # For the model of detection (p)
    ...
    sd.betalp ~ dunif(0, 6)  # constant sd for all p params
  ...
  for(k in 1:M){
    for (i in 1:nSite){
      ...
      logit(psi[i,k]) <- lpsi[k] + 
        ...
        betalpsi[k,3] * Xoc[i,3] +  # slope
        betalpsi[k,4] * Xoc[i,4]    # forest_type ! binary
      ...
      for(j in 1:nVisits){
        ...
        logit(p[i,j,k]) <- lp[k] + 
          ...
          # betalp[4] * Xob[i,j,4]   # n_observers
...
Nadd <-  26 # based on individual rarefaction curves for total forest species
...
Xoc <- Xoc[,1:4]    # 1=elevation 2=fr_dis  3=slope 4=forest_type
Xob <- Xob[,,1:3]
...
# ni <- 2500   ;   nt <- 10   ;   nb <- 0   ;   nc <- 3
ni <- 30000   ;   nt <- 5   ;   nb <- 10000   ;   nc <- 3
# (ni - nb)/nt # samples per chain = 2000 in the final models
...
# model diagnostics
Rhat is metric of between- and within-chain estimates. Typically want this to be <1.1 to indicate convergence. 
Typically want effective sample size to be >100 to accept the sample.
```{r MCMCvis}
...
MCMCtrace(samples, 
          ...
          filename = paste0("Results/", version_folder,"plots_traceDensity_pg1_forest_", format(Sys.time(), "%Y%m%d"), ".pdf"))
...
```{r save results}
saveRDS(samples, paste0(results_folder, version_folder, "BirdOccMod_dOcc_samples_forest_",format(Sys.time(), "%Y%m%d"),"_seed", mySeed, ".RDS"))
saveRDS(list(data,constants), paste0("Data/Forest_nimbleOccData_v6_", format(Sys.time(), "%Y%m%d"), ".RDS"))
```
