---
title: "bayesBirdModel_multiSPecies_v7 - forest"
author: "Liz Law"
date: "22 Mar 2022"
...
* assumed that all rare species (observed <1 in all study) have the same same beta parameters. Could vary this depending on which groups they are in.
  ...
  # Priors
    ...
    ### first make the rare param
     ...
     lpRARE ~ dnorm(mu.lp, sd=sd.lp)
      for(lpi in 1:np){
        betalpRARE[lpi] ~ dnorm(mu.betalp[lpi], sd=sd.betalp) #[lpi])
      }
...
```{r save results}
saveRDS(samples, paste0("BirdOccMod_samples_forest_", myversion,"_", format(Sys.time(), "%Y%m%d"), ".RDS"))
```
...
MCMCtrace(samples, 
          ...
          filename = paste0("plots_traceDensity_pg1_forest_", myversion,"_", format(Sys.time(), "%Y%m%d"), ".pdf"))
MCMCtrace(samples, 
          params = pg2,
          ind = TRUE, 
          Rhat = TRUE, n.eff = TRUE,
          filename = paste0("plots_traceDensity_pg2_forest_", myversion,"_",format(Sys.time(), "%Y%m%d"), ".pdf"))
