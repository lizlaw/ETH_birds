# project to raster- farm - rep by rep
...
for(scenario in c("CI", "FF", "BR")){
  for(constrainInputs in c(TRUE, FALSE)){
    ...
    pred_stack <- rast(paste0(results_folder, version_folder, "/",
                              scenario, "/",
                              "farm_pred_stack_", resolution,"_",scenario,".tif"))
    ...
    foreach (i = 1:nRep,
      ...
      foreach (k = 1:(rareSpp)) %do% {
        ...
        out_logitpsi <- posterior_matrix[i, paste0("lpsi", "[", k, "]")]  +
          ...
          posterior_matrix[i, paste0("betalpsi", "[", k, ", 5]")] * pred_stack[[5]]
