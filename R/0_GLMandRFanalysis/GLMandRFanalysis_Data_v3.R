...
# remove genus only species from the observation data
BirdData$ObsvData <- BirdData$ObsvData %>%  
  filter(species_code %>% str_detect("_sp$", negate = TRUE))
...
occVL_continuous <- c("elevation", "slope","twi", 
                      "sidi1ha", "sidi200", "pwv1ha", "pwv500", "fl_dis", "fl_dis85",
                      "fr_dis", "fr_dis85", "hli", "pwv2km") 
occVL_binary <- c("habitat", "farm_type", "forest_type") 
occVL_other <- c("point_id", "kebele", "hab_details")

obsVL_continuous <- c("start", "date")
obsVL_count <- c("n_observers")
obsVL_ordinal <- c("visibility", "cloud")
obsVL_binary <- c("round", "habitat", "recording")
obsVL_other <- c("point_id")

bSR <- y_all %>% 
  apply(c(1,2), sum) %>% 
  as_tibble() %>% 
  add_column(point_id = dimnames(y_all)[[1]], .before = 1) %>% 
  pivot_longer(cols = c(`1`, `2`), names_to = "round", values_to = "SR") %>% 
  mutate(round = factor(as.numeric(round))) 

OccVarsFarm <- OccVarsFarm %>% 
  mutate(
    # across(any_of(occVL_continuous), ~ scale(.x)[,1]),
    across(any_of(occVL_binary), factor)
  )
...
ObsVars <- ObsVars %>% 
  mutate(
    across(all_of(obsVL_continuous), ~ scale(as.numeric(.x))[,1]),  # turn numeric and scale
    across(all_of(obsVL_binary), factor)# ,
    # across(all_of(obsVL_count),  ~ scale(.x)[,1]),    # for now, treat these as continuous
    # across(all_of(obsVL_ordinal),  ~ scale(.x)[,1])   # for now, treat these as continuous
  )
...
frmSR <- bSR %>% 
  filter(habitat == "FARM") %>% 
  left_join(OccVarsFarm) 

fstSR <- bSR %>% 
  filter(habitat == "FOR") %>% 
  left_join(OccVarsFrst)
