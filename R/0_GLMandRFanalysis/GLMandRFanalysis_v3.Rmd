...
```{r, message = FALSE}
source("/Users/elaw/Desktop/LeuphanaProject/BirdModelling/ETH_birds/R/0_GLMandRFanalysis/GLMandRFanalysis_Data_v3.R")
```
...
* elevation: reasonably normal, and shows a small, negative relationship with SR (expected). No strong indication of the need for a quadratic term, but it could be ecologically valid.
...
* elevation: skewed right, strong positive relationship. Quadratic not advised based on this, as it shows upward (smile) trend, which would not be expected. 
...
* forest_type and fr_dis85 have pearson correlation of 0.73. Keep forest_type, because we discarded fr_dis85 above above
...
* pwv500 (log10(x+1), remove due to correlation with fl_dis)
* *fl_dis (sqrt)*
...
fffrm <- formula(SR ~ #habitat2 +
                  poly(elevation,2) + slope + farm_type + sidi1ha + fl_dis + 
...
* [>5%IncMSE]: date, start, fl_dis, elevation, farm_type, sidi1ha
...
* [>5%IncMSE]: date, elevation, fr_dis, forest_type, hli
* [<5%IncMSE]: cloud, n_observers, start, visibility, slope
...
# Full farm formula
fffrm3 <- formula(SR ~ 
                  elevation + fl_dis + farm_type + sidi1ha +
                  date + poly(start,2))

# Full forest formula
fffst3 <- formula(SR ~ 
                  elevation + fr_dis + forest_type + hli + 
                  date)
...
* SR ~ elevation + farm_type + sidi1ha + date + poly(start,2)
* family = poisson
...
date, farm_type, poly(start, 2), poly(elevation, 2), sidi1ha, fl_dis, slope, visibility
...
date elevation forest_type slope poly(start, 2) cloud
...
* GLM:    SR ~ poly(elevation, 2) + farm_type + slope + date + poly(start, 2) 
* RF:     SR ~ elevation + farm_type + fl_dis + sidi1ha + date + start
* GLM/RF: SR ~ poly(elevation, 2) + farm_type + sidi1ha + date + poly(start,2) 
* MuMIn:  SR ~ poly(elevation, 2) + farm_type + fl_dis+ sidi1ha + slope + date + poly(start, 2) + visibility

suggest using:

* *occupancy ~ poly(elevation, 2) + farm_type + slope + fl_dis + sidi1ha*
* *observation ~ poly(start, 2) + date + visibility*

*Forest variables* selected by the different approaches:

*   GLM: SR ~ elevation + forest_type + date + poly(start, 2) 
*   RF: SR ~ elevation + forest_type + fr_dis + hli + date
*   RF/GLM: SR ~ elevation + forest_type + date  
*   MuMIn: SR ~ elevation + forest_type + slope + date + poly(start, 2) + cloud 

suggest using:

* *occupancy ~ poly(elevation, 2) + forest_type + fr_dis + hli + slope*
* *observation ~ date + poly(start,2) + cloud*

Note above, we did not use elevation^2n in the GLMs as it was not the expected middle-maximum relationship, but we still want to allow species to be able to have these relationships. BUT we may want to force the 2nd beta term to be >= 0 
