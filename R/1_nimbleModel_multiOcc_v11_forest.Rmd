---
title: "bayesBirdModel_multiSPecies_v11 - forest"
author: "Liz Law"
date: "`r Sys.Date()`"
  ...
  for(k in 1:M){
    for (i in 1:nSite){
      ...
      logit(psi[i,k]) <- lpsi[k] + 
        ...
        betalpsi[k,4] * Xoc[i,4] +  # fr_dis
        betalpsi[k,5] * Xoc[i,5] #+  # hli
...
Nadd <- 22 # based on individual rarefaction curves for total forest species observed 75 of 97
...
# "elevation1" "elevation2" "forest_85"  "fr_dis" "hli" 
Xoc <- OccVarsFrst %>% select(-point_id) %>% as.matrix() %>% array(dim = dim(.))  
Xoc <- Xoc[,1:5]    

# observation variables [site, visit, vars]
Xob <- array(NA, dim = c(nSite, nVisits, dim(ObsVarsFrst)[2]-2)) 
...
Xob[,,4] <- ObsVarsFrst %>% arrange(round, point_id) %>% pull(cloud)
Xob <- Xob[,,1:4]
...
MCMCtrace(samples, 
          ...
          filename = paste0("Results/", version_folder, 
                            "plots_traceDensity_pg1_forest_", 
...
saveRDS(samples, paste0(results_folder, version_folder,
                        "BirdOccMod_dOcc_samples_forest_",
                        format(Sys.time(), "%Y%m%d"),
                        "_seed", mySeed, ".RDS"))
saveRDS(list(data,constants), paste0("Data/Forest_nimbleOccData_",data_version,"_",
