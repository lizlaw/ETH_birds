...
Then we conduct stepwise GLM (using AIC) and MuMIn dredge (using AICc) to identify two candidate GLM models. These are subject to model checks, and cross validation. The final model is chosen based on accuracy, fit to the data, and reasonable predictions. 
...
Version 2 excludes species identified to genus only. 
...
Sumarise observations, then remove species identified to genus only 
```{r}

# total observations
BirdData$ObsvData %>% nrow()   # 2768

allspp <- BirdData$ObsvData$species_code     # n = 2768
genspp <- allspp[str_detect(allspp, "_sp$")] # n = 333

allspp <- BirdData$ObsvData$species_code %>% unique %>% sort # n=159
genspp <- allspp[str_detect(allspp, "_sp$")]                 # n=24

# check that this does not result in loss of genus
# genspp
# allspp

# number of species in total
length(allspp); length(genspp)

# remove genus only species
BirdData$ObsvData <- BirdData$ObsvData %>%  
  filter(species_code %>% str_detect("_sp$", negate = TRUE))

# new total observations
BirdData$ObsvData %>% nrow()
BirdData$ObsvData$species_code %>% unique %>% length
...
BirdCounts %>% filter(habitat=="FARM") %>% pull(species_code) %>% unique %>% length
BirdCounts %>% filter(habitat=="FOR") %>% pull(species_code) %>% unique %>% length

siteCounts <- BirdCounts %>% 
  group_by(habitat, point_id) %>%
  summarise(n_spp = n()) 

farmCounts <- siteCounts %>% filter(habitat=="FARM") %>% pull(n_spp)
  median(farmCounts); mean(farmCounts); range(farmCounts)
forestCounts <- siteCounts %>% filter(habitat=="FOR") %>% pull(n_spp)
  median(forestCounts); mean(forestCounts); range(forestCounts)
...
scdf<- cdf %>% 
  select(-point_id) %>% 
  group_by(habitat) %>% 
  summarise(across(Bost_hage:Cucu_soli, sum)) %>% 
  select(-habitat) %>% 
  as.matrix() %>% 
  t()
...
We estimate that sampling in farmland was 91.5% (116 of 127) of species, and sampling of forest was 77.3% (75 of 97) of species. This gives us a total M species in each habitat to include in the Bayesian Multi-species-occupancy model, i.e. we add 11 species to farm, and 22 species to forest.

Check asymtote estimates:
```{r}
sum(farmcdf) #2278
sum(frstcdf) #1345
SCfm <- iNEXT(scdf, q=0, datatype = "abundance", endpoint = sum(farmcdf)*3 , nboot = 100)  
SCfm$iNextEst
SCfs <- iNEXT(scdf, q=0, datatype = "abundance", endpoint = sum(frstcdf)*3, nboot = 100)  
SCfs$iNextEst
```
This suggests that the Asymptote estimates are possibly biased as these are reached at more than 3x the current number of individuals sampled, in forest (Emax = 96.85026 at 3x1345) more than farmland (Emax = 126.57138 at 3x2278).
...
ggiNEXT(SCp) + geom_hline(yintercept = c(126.79526, 97.02528), lty = c(2,3)) # includes asymptote estimates
```
...
MAT <- cdf %>% 
  ...
  nInds <- MAT %>% colSums() 
  quantile(nInds)
  nEnd <- quantile(nInds, p=0.5) # the median number of individuals = 23
  
  (checkpoints <- bind_cols(nInds = nInds, 
                            point_id = cdf$point_id, 
                            habitat = cdf$habitat) %>% 
  ...
  RSR <- SRe$iNextEst$size_based %>% 
    filter(m==nEnd) %>% 
    add_column(nInds = nInds) %>% 
    mutate(bias2 = nInds < (nEnd/2),
           bias3 = nInds < (nEnd/3)) %>% 
    select(point_id=Assemblage, nInds, m, RSR = qD, bias2, bias3)
```
...
Rareify <- function(counts){
  ...
  # calculate endpoint at the median number of individuals observed & knots
  nInds <- tibble(
    nInds = MAT %>% colSums(),
    site = MAT %>% colnames()
  )
  nEnd <- quantile(nInds$nInds, p=0.5) # the median
  nK <- min(nEnd, 40) # number of intervals in curve estimates (capped at 40, likely less)
  ...
  RSR <- SRe$iNextEst$size_based %>% 
    filter(m==nEnd) %>% 
    left_join(nInds, by = c("Assemblage"="site")) %>% 
    mutate(bias2 = nInds < (nEnd/2),
           bias3 = nInds < (nEnd/3)) %>% 
    select(point_id=Assemblage, nInds, m, RSR = qD, bias2, bias3)
...
Specialists <- Specialists %>% 
  mutate(RSR = map(CountData, Rareify)) %>%   ## issue with 3, 5
  unnest_wider(RSR)
